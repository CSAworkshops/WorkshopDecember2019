...
function create() {
	...
	barriers.create(400, 568, 'ground').setScale(2).refreshBody();
...
function paintBarrier(pointer) {
	var image = this.barriers.createCursorKeys(pointer.x, pointer.y, 'Barrier');
	 this.physics.add.collider(this.barriers, image);
	 this.physics.add.collider(image, player);
